# 5-4. GitLab の Environment で apply 前の承認を設定する
...
この課題では、**GitLab の UI で Environment を設定し、手動承認用の job を追加して apply を止める**手順を行います。
...
### 1. GitLab の Environment を作成する

GitLab の UI から、次の設定を行ってください。

条件は次のとおりです。

- Environment 名は `production` にする
- Environment を作成して、`apply` job がそこへデプロイされることを意識できるようにする

### 2. pipeline 定義に手動承認用 job を追加する
...
- `apply` job に `environment: production` を設定する
...
> - 実際の運用では、`production` 環境を保護して、承認者が限定されるようにします
...
- `apply` job に `environment: production` が設定されていることを確認する
